<div class="contenedor-list-all">
    ...
    <div class="div-botones-paginacion-series">
    <?php if ($paginaActual > 1): ?>
        <a href="index.php?controller=serie&action=getAllSeries&page=<?php echo $paginaActual - 1; ?>" class="cambiarPagina"><img src="assets/Images/anterior.png" class="paginar"></a>
    <?php endif; ?>
    <?php for ($i = 1; $i <= $dataToView["totalPaginas"]; $i++): ?>
        <a href="index.php?controller=serie&action=getAllSeries&page=<?= $i ?>" <?= $i == $_GET["page"] ? 'style="font-weight:bold; pointer-events: none; "' : '' ?>>
        <div class="boton-paginacion-serie">
        
        <p><?= $i ?></p>    
        </div></a>
        <?php endfor; ?>
        <?php if ($paginaActual < $totalPaginas): ?>
        <a href="index.php?controller=serie&action=getAllSeries&page=<?php echo $paginaActual + 1; ?>" class="cambiarPagina"><img src="assets/Images/siguiente-boton.png" class="paginar"></a>
    <?php endif; ?>
    </div>
